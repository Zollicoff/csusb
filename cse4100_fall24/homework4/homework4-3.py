...
# Update Distance Vector for router 'd' at t=1 based on neighbors 'c' and 'f'
neighbors_d = ['c', 'f']
...
# Convert Distance Vector Table for router 'd' at t=0 and t=1 to a DataFrame
dv_table_t0 = pd.DataFrame([distance_vectors['d']], index=['t=0'])
dv_table_t1 = pd.DataFrame([distance_vectors['d']], index=['t=1'])
...
# Display the Distance Vector tables for t=0 and t=1
axs[2].axis('off')
dv_table = pd.concat([dv_table_t0, dv_table_t1])
table2 = axs[2].table(cellText=dv_table.values, colLabels=dv_table.columns, rowLabels=dv_table.index, cellLoc='center', loc='center')
table2.auto_set_font_size(False)
table2.set_fontsize(8)
table2.auto_set_column_width(col=list(range(len(dv_table.columns))))
axs[2].set_title("Distance Vector Table for Router d at t=0 and t=1")
