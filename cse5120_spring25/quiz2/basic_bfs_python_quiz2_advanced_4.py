...
def plot_graph_state(graph, queue, transitions=None, pause_time=1, info=""):
    ...
    ax_graph.set_title("BFS Step-by-Step Visualization")
...
def bfs(graph, start, goal):
    ...
    info = f"Starting BFS from {start} to reach {goal}..."
    ...
    while queue:
        ...
        for neighbor in graph[node]:
            if neighbor not in visited:
                ...
                # Check for the goal at the time of enqueuing.
                if neighbor == goal:
                    ...
                    finish_msg = f"Goal {goal} reached!"
                    print(finish_msg)
                    plot_graph_state(graph, list(queue), info=finish_msg)
    ...
    final_info = f"Final Expansion Order: {expansion_order}"
...
# Now print the final expansion order.
print("Final Expansion Order:", order)
